...
export async function loadAssets(onProgress) {
  ...
  let done = 0;
  const EXTRA = ['site', 'crawler', 'tree'];
  const total = MODEL_IDS.length + 1 + EXTRA.length;
  ...
  for (const name of EXTRA) jobs.push(loader.loadAsync(`assets/site/${name}.glb`).then((g) => { extras.set(name, g.scene); tick(); }));
  await Promise.all(jobs);
}

export const getHangar = () => hangar;
export const getExtra = (name) => extras.get(name);
