...
import { elements } from './flight/orbit.js';
...
export class UI {
  ...
  bindTop() {
    ...
    const acts = {
      ...
      undo: () => ed.undo(),
      redo: () => ed.redo(),
      track: () => this.trackDialog(),
    };
    $('#launch').onclick = () => { if (this.onLaunch) this.onLaunch(); };
  ...
  trackDialog() {
    ...
    $('#modal-title').textContent = 'Tracking station';
    const body = $('#modal-body');
    body.replaceChildren();
    if (!world.vessels.length) body.append(el('p', 'empty', 'Nothing out there yet. Vessels you leave in orbit or landed show up here.'));
    world.vessels.forEach((v, i) => {
      let status = 'Landed';
      if (!v.landed) { const o = elements(new THREE.Vector3().fromArray(v.r), new THREE.Vector3().fromArray(v.v)); status = `Orbit ${(o.pe / 1000).toFixed(0)} × ${(o.ap / 1000).toFixed(0)} km`; }
      const row = el('div', 'row');
      const fly = el('button', '', `${esc(v.name)}<small>${status}</small>`);
      fly.title = 'Fly this vessel';
      fly.onclick = () => { $('#modal').hidden = true; if (this.onFly) this.onFly(i); };
      const del = el('button', '', '✕');
      del.title = 'Remove from the world';
      del.onclick = () => { const w = loadWorld(); w.vessels.splice(i, 1); saveWorld(w); this.trackDialog(); };
      row.append(fly, del);
      body.append(row);
    });
    $('#modal').hidden = false;
