...
export class Editor {
  constructor(canvas) {
    ...
    this.hoverId = null;
    this.candidate = null;
    this.cam = { theta: 0.6, phi: 1.3, dist: 24, offsetY: 0, targetY: 9, goalY: 9 };
  ...
  initScene() {
    ...
    this.groundGoal = 12;
    this.vesselGroup.position.y = 12;
  ...
  bind() {
    ...
    c.addEventListener('pointermove', (e) => {
      ...
      if (p.down) {
        ...
        if (p.moved) {
          if (p.button === 1) this.cam.offsetY += dy * this.cam.dist * 0.0016;
    ...
    c.addEventListener('wheel', (e) => {
      e.preventDefault();
      if (e.shiftKey) this.cam.offsetY -= (e.deltaY || e.deltaX) * 0.012;
    ...
    addEventListener('keydown', (e) => {
      ...
      else if (k === 'f') { this.cam.offsetY = 0; this.fit(); }
  ...
  click(shift) {
    ...
    if (id === this.vessel.rootId && this.vessel.size > 1) { this.emit('Root part — detach the rest first, or start a new design'); return; }
    const before = this.snapshot();
  ...
  updateGround() {
    if (!this.vessel.size) { this.groundGoal = 12; this.box = null; return; }
    this.box = this.bounds();
    this.groundGoal = PAD_TOP - this.box.min.y;
  }

  fit() {
    if (!this.box) { this.cam.dist = 24; return; }
  ...
  replicate(parent, world, theta) {
    ...
    const axis = pd.dia ? [pt.pos[0], pt.pos[2]] : [0, 0];
  ...
  updateHeld() {
    ...
    if (cand) {
      ...
      setTint(this.heldTemplate, OK, 0.22);
    } else {
      const pp = this.planePoint();
  ...
  frame() {
    ...
    g.position.y += (this.groundGoal - g.position.y) * 0.18;
    const c = this.cam;
    c.goalY = this.box ? (this.box.min.y + this.box.max.y) / 2 + g.position.y : 9;
    c.targetY += (c.goalY - c.targetY) * 0.1;
    const ty = THREE.MathUtils.clamp(c.targetY + c.offsetY, 0.5, 55);
    c.offsetY = ty - c.targetY;
